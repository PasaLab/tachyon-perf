...
public class TachyonPerfMaster {
  ...
  private static void abortAllSlaves() {
    try {
      java.lang.Runtime.getRuntime().exec(PerfConf.get().TACHYON_PERF_HOME + "/bin/tachyon-perf-abort");
    } catch (IOException e) {
      e.printStackTrace();
      LOG.error(e);
    }
  ...
  public static void main(String[] args) {
    ...
    if (!master.start()) {
      abortAllSlaves();
      System.err.println("Error when start Tachyon-Perf Master");
      LOG.error("Error when start Tachyon-Perf Master");
    }
    try {
      master.stop();
    } catch (Exception e) {
      e.printStackTrace();
      System.err.println("Error when stop Tachyon-Perf Master");
      LOG.error("Error when stop Tachyon-Perf Master", e);
    }
  }
  ...
  private boolean startMasterService() {
    try {
      mMasterServiceThread.setup(PerfConf.get().TACHYON_PERF_MASTER_HOSTNAME,
          PerfConf.get().TACHYON_PERF_MASTER_PORT);
    } catch (Exception e) {
      e.printStackTrace();
      LOG.error(e);
      return false;
